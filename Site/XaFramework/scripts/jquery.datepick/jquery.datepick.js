	...
	$.JQPlugin.createPlugin({
		...
		commands: {
			...
			today: {text: 'todayText', status: 'todayStatus', // Today's month
				...
				action: function (inst) { plugin.setDate(this, new Date()); plugin.hide(this); } //raul: select current date and close { plugin.showMonth(this); }
			},
		...
		_updateInput: function(elem, keyUp) {
			...
			if (!$.isEmptyObject(inst)) {
				...
				if (!inst.inline && !keyUp) {
					//$(elem).val(value); raul: never update the input. will update from angular!!!
				}
			...
			for (var week = 0; week < numWeeks; week++) {
				...
				for (var day = 0; day < 7; day++) {
					...
					if (inst.options.rangeSelect && inst.selectedDates.length > 0) {
					    ...
					    if (inst.selectedDates.length == 0) { //raul: highlight default date
					        if (inst.get('defaultDate') && inst.get('defaultDate').getTime() === drawDate.getTime()) {
					            selected = true;
					            //break;
					        }
					    }
